...
import { Plus, Car, Edit2, Trash2, ShieldCheck, ShieldX } from "lucide-react";
import { getMeusVeiculos, deleteVeiculo, getApolices } from "../../services/Service";
...
export function VeiculoList() {
  ...
  const [veiculoParaDeletar, setVeiculoParaDeletar] = useState<any | null>(null);
  ...
  const getTipoSeguro = (veiculoId: number) => {
    ...
    return apolice.tipoSeguro || apolice.tipo || apolice.tipo_plano || apolice.plano || null;
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0a0a0a] text-white">
        <h2 className="text-2xl font-bold mb-4">Acesse sua conta para ver seus veículos</h2>
        <p className="mb-6 text-gray-400">Faça login para visualizar e gerenciar seus veículos.</p>
        <button
  ...
  return (
      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white py-10 px-4">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-400">Gerencie seus veículos cadastrados</p>
          </div>
          <button 
        ...
        {veiculos.length === 0 ? (
          <div className="text-center py-20 bg-[#1a1a1a] rounded-xl border border-[#333]">
            ...
            <h3 className="text-xl font-medium text-white">Nenhum veículo cadastrado</h3>
            <p className="text-gray-400 mt-2">Comece adicionando um novo veículo.</p>
          </div>
          ...
          <div className="text-center py-16 bg-[#1a1a1a] rounded-xl border border-[#333]">
            ...
            <h3 className="text-lg font-medium text-white">Nenhum veículo encontrado</h3>
            ...
            {veiculosFiltrados.map((veiculo) => (
              ...
              >
                ...
                <div className="flex gap-2 pt-4 border-t border-[#333]">
                  ...
                  >
                    {deletingId === veiculo.id ? "Deletando..." : <><Trash2 className="w-4 h-4" /> Deletar</>}
                  </button>
                </div>
              </div>
            ))}
    ...
    <AlertDialog open={showDialogDeletar} onOpenChange={setShowDialogDeletar}>
      <AlertDialogContent className="bg-[#1a1a1a] border-[#333]">
        <AlertDialogHeader>
          <AlertDialogTitle className="text-white text-2xl">Deletar Veículo?</AlertDialogTitle>
