...
export function Navbar() {
  ...
  return (
    <nav className="bg-black border-b border-[#4c4b4b] text-[#e6e6e6]">
          ...
          <div className="hidden md:flex items-center space-x-4">
            {isAuthenticated ? (
              ...
              <div className="flex items-center space-x-2">
                <Link to="/login" className="px-4 py-2 text-sm font-medium text-[#e6e6e6] hover:text-[#e24f10] transition-colors">Entrar</Link>
                <Link to="/cadastro" className="px-4 py-2 text-sm font-medium bg-[#e24f10] text-white rounded-md hover:bg-[#c84410] transition-colors">Cadastrar</Link>
              </div>
            )}
          </div>
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            {isAuthenticated ? (
              <>
                ...
                <button onClick={handleLogout} className="w-full text-left px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:bg-[#1a1a1a] hover:text-[#e24f10] flex items-center space-x-2">
                  <LogOut className="h-4 w-4" />
                  <span>Sair</span>
                </button>
              </>
              ...
              <div className="border-t border-[#4c4b4b] mt-2 pt-2 space-y-1">
                <Link to="/login" onClick={() => setIsOpen(false)} className="block px-3 py-2 rounded-md text-base font-medium text-gray-300">Entrar</Link>
                <Link to="/cadastro" onClick={() => setIsOpen(false)} className="block px-3 py-2 rounded-md text-base font-medium bg-[#e24f10] text-white">Cadastrar</Link>
              </div>
            )}
          </div>
        </div>
